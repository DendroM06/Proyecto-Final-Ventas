...
function table(id_pedido, total, fecha_pedido, fecha_entrega, estado) {
  if (estado == "Pendiente") {
    return (
      "<tr><td>" +
      id_pedido +
      '<td><a href="#" data-toggle="modal" data-target="#modalTask" onclick="viewDataPedido(\'' +
      id_pedido +
      "','" +
      estado +
      "')\">" +
      ...
      estado +
      "</td>" +
      '<td><a href="#" data-toggle="modal" data-target="#modalDelete" onclick="idEliminarPedido(\'' +
      id_pedido +
      "')\">" +
      ...
      if (value.id_pedido == id_pedido) {
        ...
        reference2.on("value", function (datos2) {
          ...
          document.getElementById(
            "labelSubtotal"
          ).innerText = _subtotal.toFixed(2);
          document.getElementById("labelIva").innerText = (
            _subtotal * 0.12
          ).toFixed(2);
          document.getElementById("labelTotal").innerText = (
            _subtotal +
            _subtotal * 0.12
          ).toFixed(2);
          var sendData = table2(
